...
// Code taken from the Apache Flink project: http://git.io/vCTOl
 ...
 * values are stored in a [[scala.collection.immutable.Map]] being identified by a [[Parameter]] object.
 * ParameterMaps can be fused. This operation is left associative, meaning that latter
 * ParameterMaps can override parameter values defined in a preceding ParameterMap.
 *
...
trait Parameter[T] {
  ...
  /**
   * Default value of parameter. If no such value exists, then returns [[scala.None]]
   * When extending Parameter[T] remember to add the default value as Some(defaultValue)
   * since it should be of type an Option[T].
   */
