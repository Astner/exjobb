...
object CorrelationMeasures {
  ...
  * If a confidence level is specified, the (exact) variance of the estimate is also calculated.
  ...
  * 0.99, 0.999 etc. Values less than 0.5 lead to a more speculative measure, i.e. it will tend to estimate
  ...
  */
  ...
  (Double, Double) = {
    ...
    val pwexp = Gamma.digamma(nij + 1.0) -
                Gamma.digamma(ni + 1.0) -
                Gamma.digamma(nj + 1.0) +
                Gamma.digamma(n + 1.0)

    if(confidence.isDefined) {
      // Exact value for variance of pwmi
      val pwvar = Gamma.trigamma(nij + 1.0) +
                  Gamma.trigamma(ni + 1.0) +
                  Gamma.trigamma(nj + 1.0) +
                  3.0 * Gamma.trigamma(n + 1.0)
