...
class CGraph(
  ...
  lazy val edgeWeightTuples = edges
    ...
    .map { case (j, (((i, wi), wij), wj)) => (i, j, wi, wj, wij)}.cache().setName("edgeWeightTuples")
  ...
  lazy val prunedEdges = edgesAndWeightSums._1.cache().setName("prunedEdges")
  lazy val discardedEdgeWeightSums = edgesAndWeightSums._2.cache().setName("discardedEdgeWeightSums")
  lazy val remainingEdgeWeightSums = edgesAndWeightSums._3.cache().setName("remainingEdgeWeightSums")
  ...
  lazy val sharedTermsWithSums: RDD[(Long, Long, Double, Double, Double, Double, Double)] = {
    ...
    val inEdgesPerVertex = prunedEdges
      ...
      .cache().setName("sharedTermsWithSums")
    ...
    // TODO: discardedEdgeWeightSums and remainingEdgeWeightSums are tiny and should bcase joined instead.
    // Is it possible to combine the two joins into one bcast operation?
    // Join with sums of kept and discarded weights per vertex
    sharedTerms
      // First add i's sums
      .map { case ((i: Long, j: Long), sij: Double) => (i, (j, sij))}
      .cache().setName("sharedTermsMapped")
